...
\brief Constants and types used for command creation and parsing
*/

#ifndef __COMMANDPARAMETERS_H__
#define __COMMANDPARAMETERS_H__

//! The max length of an unparsed command string
#define MAX_CMD_LEN 1024
//! The max length of the message command string (depends on the LCD row size)
#define CMD_MSGLEN 20
//! The max length of a header/menu string
...
/** 
  ...
  by the '@' character that is recognized by the parser as the command separator\n
  
  Example: <cmd 1>@<cmd 2>@<cmd 3> \n
*/
#define CMD_SEPARATOR '@'

//! Null character used as string termination
#define CMD_NULLCHAR '\0'
...
//! The length  of CMD_CHARACTERS + 1
...
/**
  ...
  out of bound string length , wrong cursor position etc. never occurs using the
...
/**
  ...
  the multi-language localization.\n
  name: r \n
  usage: r;<parameter ID> \n
  direction: send\n
  \todo Should be implemented in the RPI master before
  \todo Update the definitino in the ChipKit class header accordingly.
  */
#define CMD_MASTER_REQUEST 'r'
